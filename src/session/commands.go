...
func (u *User) useWho() {
	...
	infos := u.Server.ListOnlineUsers()
	for _, info := range infos {
		msg := "[" + info.Addr + "]" + info.Name + ":在线中...\n"
		u.SendMsg(msg)
	}
}
...
func (u *User) useRename(msg string) {
	...
	// attempt rename via server registration swap
	if u.Server == nil {
		return
	}
	// unregister old name then register new name
...
func (u *User) useRead(msg string) {
	...
	if u.Server != nil {
		u.Server.HandleReadAck(u.Name, serverMsgID)
	}
...
// 群组相关操作代理到 ServerAPI 的 GroupManager
