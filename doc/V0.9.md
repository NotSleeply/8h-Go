...
3. **业务封包与拆包链路**
   ...
   * **服务端协议拆包**：服务端的 `user.DoMessage()` 处理提取到的字符串，利用 `strings.Split` 和 `strings.HasPrefix` 等手段解析特定命令头（如 `who`, `rename|`, `to|`），抽离出业务意图与参数，进而执行在线查找、遍历转发等逻辑。
