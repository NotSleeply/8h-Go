...
func (s *InMemoryStore) ScheduleRetry(serverMsgID, to string, lastErr error, maxRetries int, baseBackoff time.Duration) (bool, error) {
		...
		if e, ok2 := m[to]; ok2 && e != nil {
			...
			if e.Retry >= maxRetries {
				...
				if err := storage.DB.Model(&storage.MessageRecipient{}).
					...
					Updates(map[string]any{"status": 3, "retry_count": e.Retry, "last_error": errText, "next_retry_at": nil}).Error; err != nil {
					return true, err
				}
			...
			e.NextRetryAt = now.Add(baseBackoff * time.Duration(1<<exp)).Unix()
		}
	}
	s.mu.Unlock()
	...
	if nextRetryCount >= maxRetries {
		if err := storage.DB.Model(&storage.MessageRecipient{}).Where("id = ?", row.ID).
			Updates(map[string]any{"status": 3, "retry_count": nextRetryCount, "last_error": errText, "next_retry_at": nil}).Error; err != nil {
			return false, err
		}
		if c := cache.Client(); c != nil {
			ctx := context.Background()
	...
	nextRetryAt := now.Add(baseBackoff * time.Duration(1<<exp))
	if err := storage.DB.Model(&storage.MessageRecipient{}).Where("id = ?", row.ID).
		Updates(map[string]any{"retry_count": nextRetryCount, "last_error": errText, "next_retry_at": &nextRetryAt}).Error; err != nil {
		return false, err
	}
	if c := cache.Client(); c != nil {
		ctx := context.Background()
