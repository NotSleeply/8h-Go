...
func (s *InMemoryStore) SaveDelivery(serverMsgID, to string) error {
	...
	row := &storage.MessageRecipient{ServerMsgID: serverMsgID, ToUser: to, Status: 0}
...
func (s *InMemoryStore) MarkDeliverySent(serverMsgID, to string, sendErr error) error {
	...
	updates := map[string]any{"retry_count": clause.Expr{SQL: "retry_count + 1"}, "last_send_at": time.Now()}
	if sendErr != nil {
...
func (s *InMemoryStore) MarkDeliveryRead(serverMsgID, to string) error {
	...
	if err := storage.DB.Model(&storage.MessageRecipient{}).
		Where("server_msg_id = ? AND to_user = ?", serverMsgID, to).
		Updates(map[string]any{"status": 2, "acked_at": &now, "read_at": &now}).Error; err != nil {
		return err
	}
	if c := cache.Client(); c != nil {
		ctx := context.Background()
...
func (s *InMemoryStore) markRecipientRowsDeadIfMessageMissing(serverMsgID, reason string) {
	...
	_ = storage.DB.Model(&storage.MessageRecipient{}).
		...
		Updates(map[string]any{"status": 3, "last_error": reason, "next_retry_at": nil}).Error
}
